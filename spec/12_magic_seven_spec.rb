...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the random number is 10 and previous step is 10' do
      subject(:ten) { described_class.new(10) }
      it 'returns 0' do
        previous_step = 10
        result = ten.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 0' do
      subject(:zero) { described_class.new(0) }
    ...
    context 'when the random number is -100' do
      subject(:poop) { described_class.new(-100) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = poop.play
        expect(result).to eq(7)
    ...
    context 'when the random number is 100' do
      subject(:pee) { described_class.new(100) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = pee.play
        expect(result).to eq(7)
