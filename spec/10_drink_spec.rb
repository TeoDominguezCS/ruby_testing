...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:pee) { described_class.new("pee") }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(pee.type).to eq('pee')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(pee).to be_full
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:poop) { described_class.new() }
      # remove the 'x' before running this test
      it 'is full' do
        expect(poop).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:poopy) { described_class.new("poopy", 1)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(poopy).to_not be_full
      end
    end
  end
end
